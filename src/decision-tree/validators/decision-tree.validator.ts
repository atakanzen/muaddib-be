...
import {
    DecisionTreeEdgeType,
    DecisionTreeNodeType,
    RawDecisionTree,
    RawDecisionTreeEdge,
    RawDecisionTreeNode,
    RawDecisionTreeViewport,
} from '../types/decision-tree.types';

const decisionTreeNodeTypes = Object.values(DecisionTreeNodeType);
const decisionTreeEdgeTypes = Object.values(DecisionTreeEdgeType);

export function validateDecisionTree(tree: unknown): {
    error: string;
    validatedTree?: RawDecisionTree;
} {
...
function validateNodes({
    ...
    for (const node of nodes) {
        if (!isObject<DeepPartial<RawDecisionTreeNode>>(node)) {
            return {
                error: 'Decision tree "nodes" elements must all be objects',
            };
        }
        ...
        if (nodeIds.has(node.id)) {
            return {
                error: `Decision tree "nodes" has duplicate id "${node.id}"`,
            };
        }
        ...
        if (!decisionTreeNodeTypes.some((t) => node.type === t)) {
            return {
                error: `"type" in decision tree "nodes" must equal to one of [${decisionTreeNodeTypes.join(' ')}]`,
            };
        }
        ...
        if (node.position === undefined) {
            return {
                error: '"position" in decision tree "nodes" must be defined',
            };
        }

        if (!isNumber(node.position.x)) {
            return {
                error: '"position.x" in decision tree "nodes" must be a number',
            };
        }

        if (!isNumber(node.position.y)) {
            return {
                error: '"position.y" in decision tree "nodes" must be a number',
            };
        }
        ...
        if (node.measured === undefined) {
            return {
                error: '"measured" in decision tree "nodes" must be defined',
            };
        }

        if (!isNumber(node.measured.width)) {
            return {
                error: '"measured.width" in decision tree "nodes" must be a number',
            };
        }

        if (!isNumber(node.measured.height)) {
            return {
                error: '"measured.height" in decision tree "nodes" must be a number',
            };
        }
...
function validateEdges({
    ...
    for (const edge of edges) {
        if (!isObject<DeepPartial<RawDecisionTreeEdge>>(edge)) {
            return {
                error: 'Decision tree "edges" elements must all be objects',
            };
        }
        ...
        if (edgeIds.has(edge.id)) {
            return {
                error: `Decision tree "edges" has duplicate id "${edge.id}"`,
            };
        }
        ...
        if (
            edge.type !== undefined &&
            !decisionTreeEdgeTypes.some((t) => edge.type === t)
        ) {
            return {
                error: `"type" in decision tree "edges" must equal to one of [${decisionTreeEdgeTypes.join(' ')}]`,
            };
        }
        ...
        if (!isString(edge.source)) {
            return {
                error: '"source" in decision tree "edges" must be a string',
            };
        }

        if (!nodeIds.has(edge.source)) {
            return {
                error: `One of the edge sources is "${edge.source}", but no such id is found in nodes`,
            };
        }

        if (!isString(edge.target)) {
            return {
                error: '"target" in decision tree "edges" must be a string',
            };
        }

        if (!nodeIds.has(edge.target)) {
            return {
                error: `One of the edge targets is "${edge.target}", but no such id is found in nodes`,
            };
        }

        if (edge.source === edge.target) {
            return {
                error: `One of the edges has same source and target "${edge.source}"`,
            };
        }

        if (edge.animated !== undefined && !isBoolean(edge.animated)) {
            return {
                error: '"animated" in decision tree "edges" must be a boolean or undefined',
            };
        }
