...
export class DecisionTreeService {
    ...
    async update({ userId, treeId, values }: {
        userId: number;
        treeId: string;
        values: {
            name?: string;
        ...
        if (values.name === undefined && values.tree === undefined) {
            throw new BadRequestException('Request must contain at least one of values: ["name", "tree"]');
        }

        const affected = await this.db.update(decisionTrees)
            .set(values)
            .where(
                and(
                    eq(decisionTrees.userId, userId),
    ...
    async delete({ userId, treeId }: {
        userId: number;
        treeId: string
    }) {
        const affected = await this.db.delete(decisionTrees)
            .where(
                and(
                    eq(decisionTrees.userId, userId),
                    eq(decisionTrees.id, treeId)
